class zen_context_menu_actions {
    // Both are master switches: while ON, the overlay follows the curator's live
    // selection instead of the current context-menu selection, so the condition
    // is a plain setting check rather than an object test. The setting is read
    // LIVE and the machinery is registered unconditionally (see XEH_postInit), so
    // flipping either mid-mission just works.
    //
    // Statement, condition and modifierFunction are the SHARED stream-engine
    // halves parameterised by stream id — a third overlay is one more block here
    // plus a gather/draw pair, not another copy of the machinery.
    class RTZ_Overlays {
