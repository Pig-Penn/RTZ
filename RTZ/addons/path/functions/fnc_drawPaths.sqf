...
/*
 * Author: Maxim
 * RENDER_WORLD renderer for an open planning session: one drag handle per path,
 ...
 * is skipped outright while the Zeus map covers the 3D view — FUNC(drawPathsMap)
 * takes over there.
 *
 ...
 * cursor, not to whichever path happened to be seeded first.
 *
 * The line is decimated to a per-path budget that falls off with camera
 * distance, which is what keeps a curator with a platoon's worth of long paths
 ...
 *
 * Example:
 * [_ctx] call rtz_path_fnc_drawPaths
 *
 * Public: No
 */
...

{
    ...
    // Rebuilt per path per frame rather than mutating the stored colour in
    // place: the record's colour is the path's identity and must survive being
    // hovered.
    ...
    drawIcon3D [
        ...
        "", 0, TEXT_SIZE, PATH_FONT, "center", false, 0, 0
    ];
    ...
    if (_dist < LABEL_MAX_DIST) then {
        drawIcon3D [
            ...
            0, 0, 0, _label, 1, TEXT_SIZE, PATH_FONT, "center", false, 0, 0
        ];
    };
    ...
    if (_kind == KIND_AIR) then {
        ...
        drawIcon3D [
            ...
            0, 0, 0, _alt, 1, TEXT_SIZE, PATH_FONT, "center", false, 0, 0
        ];
    };
    ...
    // Budget falls off with distance; one divide per path, no tables
    ...
    for "_i" from 0 to _last step _stride do {
        ...
        // Cut pick. Records the INDEX as well as the position, so FUNC(cutPath)
    ...
    // Decimation can step past the true end; close the line so the drawn path
