 ...
 * disabling PATH would make FUNC(pathKind) reject the very units this froze, so
 * a frozen selection could never be committed; and doStop is what the executor
 * issues first in any case, so a held unit that then receives a path transitions
 * without a release-and-re-stop in between.
 *
 * The release is doFollow, which is also how a unit is returned to formation
 * after a path finishes. Worth knowing: this will also release a unit that
 ...
 *
 * Example:
 * [_unit, true] call rtz_path_fnc_setHold
 *
 * Public: No
 */
