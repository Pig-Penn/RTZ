...
/*
 * Author: Maxim
 * Opens a path planning session for the current Zeus selection: seeds one path
 * record per pathable entity, installs the mode's input handlers and starts its
 * tick.
 *
 ...
 * there is no reason for the server to hear about a path being drawn.
 *
 ...
 *
 * Example:
 * call rtz_path_fnc_beginPlanning
 *
 * Public: No
 */
...
// here and are dropped by the objectParent test — they are pathed as their
...

{
    ...
    if (count _paths >= MAX_PATHS) then {break};

    private _hull = _x;
    private _kind = [_hull] call FUNC(pathKind);
...
if (_paths isEqualTo []) exitWith {
    ...
    [LLSTRING(MsgNonePathable)] call EFUNC(common,showCountMessage);
    false
};
...
// while a path is being traced it is the thing being interacted with.
[QGVAR(paths3D), LINKFUNC(drawPaths), RENDER_WORLD, 65] call EFUNC(core,registerRenderer);
...
if (!isNull _mapCtrl) then {
    GVAR(mapEH) = _mapCtrl ctrlAddEventHandler ["Draw", {_this call FUNC(drawPathsMap)}];
};
...
// Optional soft hold so the selection does not walk out from under the paths
// being drawn for it. doStop, NOT Wargame's disableAI "PATH": disabling PATH
// would make FUNC(pathKind) reject the very units this froze, and doStop is
...
if (GETGVAR(freezeWhilePlanning,false)) then {
    {
        private _unit = _x select PATH_UNIT;
