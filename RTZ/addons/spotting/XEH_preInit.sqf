...
// GVAR(enableSpottingSystem)) while some readers of these are not. Same reasoning as
// rtz_hud's and rtz_officer's preInit blocks.
...
// creates it behind the RC setting, while FUNC(drawSpots) — registered by
...
// unconditionally, it is now a plain read — matching FUNC(drawRcIndicator),
