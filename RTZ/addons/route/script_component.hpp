...
#define COMPONENT_BEAUTIFIED Route
#include "\x\rtz\addons\main\script_mod.hpp"

// #define DEBUG_MODE_FULL
// #define DISABLE_COMPILE_CACHE

#ifdef DEBUG_ENABLED_ROUTE
    #define DEBUG_MODE_FULL
#endif

#ifdef DEBUG_SETTINGS_ROUTE
    #define DEBUG_SETTINGS DEBUG_SETTINGS_ROUTE
#endif

#include "\x\rtz\addons\main\script_macros.hpp"

// Route polylines and drag handles draw through rtz_core's ONE Draw3D handler,
...
// Decided ONCE per entity by FUNC(routeKind) and stored in the record, because
...
// executor. KIND_NONE doubles as the "not routable" answer, so one call filters
...
// ── Layout of a planned route in GVAR(routes) ────────────────────────────────
// One record per entity being planned, on the CURATOR'S CLIENT only — nothing
// here is authoritative and nothing is broadcast until FUNC(commitRoutes) runs.
...
// (hover, formation offset) are a findIf over at most MAX_ROUTES entries. Same
...
// unambiguous choice over water, which matters the moment boats are routable.
//
...
// from the tail of POINTS so the renderer does not branch on "is this route
// still empty?" for every route, every frame.
//
...
#define ROUTE_UNIT    0
#define ROUTE_HULL    1
#define ROUTE_POINTS  2
#define ROUTE_COLOR   3
#define ROUTE_KIND    4
#define ROUTE_HEAD    5
#define ROUTE_DIR     6
#define ROUTE_LABEL   7
...
// route while one is in flight, and it expires on its own so a search the engine
// never answers cannot wedge the route permanently.
#define ROUTE_SEARCH  9
// The unit whose route this one is trailing in formation, or objNull. Set when a
// route starts trailing and cleared the moment the curator grabs that handle
// himself — a route somebody has drawn on by hand is never overwritten.
#define ROUTE_LEADER  10
...
// executor cannot use — and it is what turns a 200 m route from 800 points into
...
// COMMIT_SPACING is the OTHER sampling rate. A route is drawn dense and
...
// worth of committed routes from being tens of thousands of numbers on the wire.
...
// Gap between a route and the one trailing it in formation (meters), converted
// to a count of drawn points by FUNC(formationTrail)
#define PROF_FORMATION_GAP  9

// ── Aircraft altitude drag ──────────────────────────────────────────────────
// An aircraft route is drawn in two gestures: dragging the handle moves it
...
// 25 m spacing an aircraft route is traced at is right for flying across a map
...
// ── Road magnetisation ──────────────────────────────────────────────────────
// Holding the modifier while tracing a vehicle route pulls each sample onto the
// nearest road, which is what makes a convoy route drawable at all — freehand
...
// would teleport the route (meters)
...
// compound and have the route go round it, or through its doors, without
// hand-tracing every corner.
//
// BOUNDED. A search that the engine never answers must not wedge the route or
...
// without this the route grows a straight leg through everything in between,
...
// WATER, where a land route is either crossing a bridge or is invalid — this is
// how far above and below the point that ray looks (meters).
#define BRIDGE_PROBE 3

// ── Layout of a route being followed, in GVAR(active) ───────────────────────
// One record per unit currently executing a route ON THIS MACHINE — the one the
...
// instead of examining every route on the same tick.
//
...
// How far the AI's own plan may drift from this route's current leg before the
...
// that slow before its route is abandoned as stuck (seconds). STUCK_TIME is
// deliberately long: a unit that stops for twenty seconds is usually taking
// cover or returning fire, and cancelling a route for that would punish exactly
...
// A route whose ends meet is a patrol: it commits as a loop instead of a
...
// and hits the key gets the first MAX_ROUTES of them rather than a client that
...
#define ROUTE_FONT  "RobotoCondensed"

// The route's start is the same dot as every other point, drawn larger, rather
...
// smaller one that keeps the route line off the ground it follows (meters).
...
// Alpha for a route that is neither hovered nor being dragged, and for one that
...
// Line segments a single route may draw per frame, before distance reduces it.
// A route at DRAW_NEAR metres or closer gets the full budget; further out the
// budget falls off with distance to DRAW_MIN, because a route two kilometres
...
// Colour of the cut marker drawn on the route point the cursor would remove
#define COLOR_CUT [0.95, 0.15, 0.15, 0.9]

// How often FUNC(planTick) re-checks that its subjects are still routable
