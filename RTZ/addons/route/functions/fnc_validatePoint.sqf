...
/*
 * Author: Maxim
 * Whether a route may be extended from one position to another: is the segment
 ...
 * must not veto a route through it. It deliberately does NOT try to recognise
 ...
 * faces, so the only question left is water: a boat route must be on it, a land
 * route must not — unless something solid is spanning it, which is a bridge or a
 * pier, and is the one case that pays for a ray.
 *
 * Arguments:
 * 0: Segment start, ASL <ARRAY>
 * 1: Segment end, ASL <ARRAY>
 * 2: Hull being routed, ignored by the line test <OBJECT>
 * 3: Route kind, one of KIND_* <NUMBER>
 * 4: Drawing profile for that kind <ARRAY>
 *
 * Return Value:
 * The route may be extended to the end position <BOOL>
 *
 * Example:
 * if ([_head, _target, _hull, _kind, _profile] call rtz_route_fnc_validatePoint) then {...};
 *
 * Public: No
 */
