...
/*
 * Author: Maxim
 * Puts a committed route into execution. Runs where the unit is local — the only
 * machine an AI order has any effect on — reached by the targeted event
 * FUNC(commitRoutes) sends.
 *
 ...
 * is pulled back out of its route by the group leader within seconds.
 *
 * Two executors, not four:
 *
 *  - Land vehicles get setDriveOnPath, and the engine drives. There is no
 *    ongoing work at all — the record exists only so the route can be noticed
 ...
 * 2: Route legs, ASL <ARRAY>
 * 3: Route kind, one of KIND_* <NUMBER>
 * 4: Cycle the route instead of stopping at the end <BOOL>
 *
 * Return Value:
 * None
 *
 * Example:
 * [_unit, _hull, _legs, KIND_INFANTRY, false] call rtz_route_fnc_startFollow
 *
 * Public: No
 */
...
if ([_hull] call FUNC(routeKind) == KIND_NONE) exitWith {};
...
// Created by the first route, destroyed by the last (FUNC(followTick)) — the
...
// Everything that is not a land vehicle walks, flies or sails the route on the
