...
/*
 * Author: Maxim
 * Offers one cursor sample to a route. Most samples are refused — the cursor
 ...
 *
 * Arguments:
 * 0: Route record, mutated in place <ARRAY>
 ...
 *
 * Example:
 * [_route, _cursorASL] call rtz_route_fnc_appendPoint
 *
 * Public: No
 */
...
_route params ["", "_hull", "_points", "", "_kind", "_head"];

private _profile = GVAR(profiles) select _kind;

// Bounded. A route that has run out of length simply stops growing; the drawn
...
if (!_snapped) then {
    ...
    if !([_head, _target, _hull, _kind, _profile] call FUNC(validatePoint)) exitWith {
        ...
        // through; the route continues on its own once the answer arrives
        // (FUNC(splicePath)).
        [_route, _target] call FUNC(autoPath);
    };
...
// May drop points off the tail and move ROUTE_HEAD backwards with them, so the
...
[_route, _target, _profile] call FUNC(smoothTail);

_points pushBack _target;
_route set [ROUTE_DIR, (_route select ROUTE_HEAD) getDir _target];
_route set [ROUTE_HEAD, _target];
...
if (_kind == KIND_AIR) then {
    _route set [ROUTE_ALT, format ["%1 m", round ((ASLToAGL _target) select 2)]];
};

// Everything else in the selection follows this route, a fixed distance behind,
// so a column is one gesture rather than six
if (GETGVAR(formationTrail,true) && {(_route select ROUTE_UNIT) isEqualTo GVAR(grabbed)}) then {
    [_route] call FUNC(formationTrail);
};
