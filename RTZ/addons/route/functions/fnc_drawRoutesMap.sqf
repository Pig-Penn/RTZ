...
/*
 * Author: Maxim
 * The Zeus-map half of the planning display. Same routes, same handles, same
 * picks — drawn with the map control's own primitives, which is the only thing
 * that genuinely differs between this and FUNC(drawRoutes).
 *
 ...
 * safe, and is what lets a route be traced, picked and cut on the map exactly as
 ...
 *
 * Example:
 * [_map] call rtz_route_fnc_drawRoutesMap
 *
 * Public: No
 */
...

{
    ...
    // takes ROUTE_DIR unmodified
    _map drawIcon [
        ICON_HANDLE, _drawColor, _handlePos,
        _size, _size, _dir, _label, 1, TEXT_SIZE, ROUTE_FONT, "center"
    ];
    ...
    if (_kind == KIND_AIR) then {
        _map drawIcon [
            ...
            0, 0, 0, _alt, 1, TEXT_SIZE, ROUTE_FONT, "right"
        ];
    };
    ...
    _map drawIcon [
        ...
        _originSize, _originSize, 0, "", 0, TEXT_SIZE, ROUTE_FONT, "center"
    ];

    // No distance falloff here — the map shows the whole route at once, and
    ...
    for "_i" from 0 to _last step _stride do {
        ...
        if (_drawn % DOT_EVERY == 0) then {
            _map drawIcon [
                ...
                MAP_POINT_SIZE, MAP_POINT_SIZE, 0, "", 0, TEXT_SIZE, ROUTE_FONT, "center"
            ];
        };
...
if (_hoveredPoint isNotEqualTo []) then {
    _map drawIcon [
        ...
        MAP_ICON_SIZE, MAP_ICON_SIZE, 0, "", 0, TEXT_SIZE, ROUTE_FONT, "center"
    ];
};
