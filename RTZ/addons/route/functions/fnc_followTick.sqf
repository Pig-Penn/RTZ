...
/*
 * Author: Maxim
 * The engine behind every route running on this machine: ONE shared handler
 * walking GVAR(active), created by the first route (FUNC(startFollow)) and
 * removed by the last. Between routes the handler does not exist, so the idle
 ...
 * inside that — which also means routes are examined a few at a time rather than
 ...
 * off this route's current leg, which is exactly when it was stolen and never
 ...
 *
 * Example:
 * [rtz_route_fnc_followTick, TICK_INTERVAL] call CBA_fnc_addPerFrameHandler
 *
 * Public: No
 */
...
// Last route finished on the previous pass — take the loop down rather than
...
// Backwards, so a finished route can be deleted without disturbing the indices
...
for "_i" from (count _active) - 1 to 0 step -1 do {
    ...
    private _finished = !alive _unit
        || {!alive _hull}
        // Ownership moved mid-route (rtz_control's transfer, a JIP handover, a
        ...
        // this route so teardown releases the unit it actually stopped
