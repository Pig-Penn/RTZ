...
/*
 * Author: Maxim
 * Installs the curator-display handlers route planning listens on, and hands
 ...
 *
 * Example:
 * GVAR(inputEHs) = [_display] call rtz_route_fnc_handleInput
 *
 * Public: No
 */
...
_handlers pushBack ["MouseButtonDown", _display displayAddEventHandler ["MouseButtonDown", {
    ...
    // Ctrl-click on a route point cuts the route back to it. Tested before the
    ...
    if (_ctrl && {_point isNotEqualTo []}) exitWith {
        [_point select 0, _point select 1] call FUNC(cutRoute);
        true
    };
    ...
    // here the route is the curator's, and FUNC(formationTrail) will not touch
    // it again.
    private _index = GVAR(routes) findIf {(_x select ROUTE_UNIT) isEqualTo _hovered};
    if (_index != -1) then {
        (GVAR(routes) select _index) set [ROUTE_LEADER, objNull];
    };
...
_handlers pushBack ["KeyDown", _display displayAddEventHandler ["KeyDown", {
    ...
    // Delete clears the route under the cursor — a cut at the beginning of it.
