...
/*
 * Author: Maxim
 * Pulls a candidate point onto the nearest road, for tracing convoy routes.
 * Freehand tracing of a winding road produces a line that clips every verge;
 * this is what makes a road route drawable at all.
 *
 ...
 * point it started from and the route never advances. Wargame biases the same
 ...
 * 1: Current route head, ASL <ARRAY>
 *
 * Return Value:
 ...
 *
 * Example:
 * ([_target, _head] call rtz_route_fnc_snapToRoad) params ["_target", "_snapped"]
 *
 * Public: No
 */
...
// teleport the route sideways onto whatever else is nearby
