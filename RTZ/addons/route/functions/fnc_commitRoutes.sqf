 ...
 * where the routes were drawn and the only machine that has them.
 *
 ...
 * only machine that hears about a route is the one that will run it.
 *
 * Each route is resampled by FUNC(reducePath) before it goes out. What crosses
 * the wire is a few dozen legs, not the several hundred points the line on
 * screen was drawn from.
 *
 * A route whose ends meet commits as a PATROL and cycles instead of stopping.
 ...
 *
 * Return Value:
 * Routes ordered <NUMBER>
 *
 * Example:
 * private _n = call rtz_route_fnc_commitRoutes
 *
 * Public: No
 */
...

{
    ...
    // A route nobody drew anything on is not an order to stand still
    if (_points isEqualTo []) then {continue};
    if ([_hull] call FUNC(routeKind) == KIND_NONE) then {continue};
