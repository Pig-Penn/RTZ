...
/*
 * Author: Maxim
 * Straightens the tail of a route against a position about to be appended to it:
 ...
 * 2: Drawing profile for the route's kind <ARRAY>
 *
 * Return Value:
 * Points were dropped <BOOL>
 *
 * Example:
 * [_route, _target, _profile] call rtz_route_fnc_smoothTail
 *
 * Public: No
 */

params ["_route", "_target", "_profile"];
...
private _hull = _route select ROUTE_HULL;
...
for "_i" from ((_count - _steps) max 0) to (_count - 2) do {
    ...
    if (_clear) then {
        ...
        _points resize (_i + 1);
        _route set [ROUTE_HEAD, _points select _i];
        _cut = true;
        break;
    };
};
