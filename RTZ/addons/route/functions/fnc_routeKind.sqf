...
/*
 * Author: Maxim
 * Classifies an entity for route planning, and by doing so decides whether it
 * can be routed at all: KIND_NONE is the "no" answer, so one call filters a
 * selection and labels what survives in a single pass.
 *
 * The single definition of "routable", shared by the places that must agree on
 * it — FUNC(beginPlanning) building the session, FUNC(commitRoutes) re-checking
 ...
 * for units it does not own, and locality is checked where the route actually
 * runs.
 *
 ...
 * curator can therefore draw a route for a unit whose PATH is disabled
 ...
 *
 * Example:
 * private _kind = [_vehicle] call rtz_route_fnc_routeKind
 *
 * Public: No
 */
...
// himself and must not have a route pushed under him.
if (isNull _unit || {!alive _unit} || {isPlayer _unit}) exitWith {KIND_NONE};

// A unit whose PATH feature is off cannot navigate, so a route would be issued
...
if (_hull isKindOf "CAManBase") exitWith {
    // A man riding in something is routed through his vehicle, not on his own
...
// only thing keeping a mortar out of a driving route.
