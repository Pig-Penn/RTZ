...
//
// Routes being planned right now, one record per entity (layout in
...
// [unit, pointIndex, drawPosition] of the route point the cursor would cut back
...
// Route colours, assigned by index so two routes drawn in the same session are
// never the same colour and a given route keeps its colour for the session.
...
GVAR(profiles) = [
    ...
    // KIND_BOAT — climb is capped hard because a boat route that gains height has
    // left the water, which is the failure this catches when a curator traces
    // across a headland.
    [12, 300, 2, 1, 0.25, "GEOM", "VIEW", 6, 60, 45]
];

// ── Execution state (the machine each routed unit is local to) ───────────────
// Routes being followed right now, one record per unit (layout in
// script_component.hpp). Declared outside the recompile block so a live
// recompile swaps the functions without stranding routes already running — their
// units would never be handed back to their groups.
//
// The per-frame handler that drives them is created with the first route and
