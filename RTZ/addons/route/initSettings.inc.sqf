...
// out from under the route being drawn for them — but it is also a mid-battle
...
// Client-side: this only changes what the curator drawing the routes sees
...
// curator's client, so every machine has to agree on them or the same route
// behaves differently depending on who happens to own the unit.

// The backstop, not the normal way a route ends. A route that is going to
// complete completes; this is what stops one that never will — a vehicle wedged
// somewhere the stuck check keeps forgiving, a unit walking a route whose far
...
// config maxSpeed on a traced route drives it like a time trial and loses the
// line on the first corner; this is what makes a drawn route look driven rather
