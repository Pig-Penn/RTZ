_costs append [

    // APCs
    ["ACM_B_MSF_V_brdm_unarmed", 10],
    ["ACM_B_MSF_V_brdm", 15],
    ["ACM_B_MSF_V_M113", 15],
    ["ACM_B_MSF_V_M113_Engineering", 16],
    ["ACM_B_MSF_V_bmp1", 22],

    // Boats
    ["ACM_B_MSF_V_Transport_Boat", 2],
    ["ACM_B_MSF_V_RescueBoat", 2],
    ["ACM_B_MSF_V_Police_Boat", 3],
    ["ACM_B_MSF_V_ArmedBoat", 15], // Speedboat Minigun
    ["vn_b_boat_06_02", 30], // PTF Nasty Boat 
    ["vn_b_boat_06_01", 30], // PTF Nasty Boat 
    ["vn_b_boat_05_02", 25], // PTF Nasty Boat (Mortar)
    ["vn_b_boat_05_01", 25], // PTF Nasty Boat (Mortar)

    // Cars
    ["ACM_B_MSF_V_UAZ", 3],
    ["ACM_B_MSF_V_UAZ_Armed", 8],
    ["ACM_B_MSF_V_UAZ_SPG", 13],
    ["ACM_B_MSF_V_Ural_Transport", 4],
    ["ACM_B_MSF_V_Ural_Medic", 4],
    ["ACM_B_MSF_V_Ural_Tractor", 3],
    ["ACM_B_MSF_V_Ural_Refuel", 8],
    ["ACM_B_MSF_V_Ural_Repair", 9],
    ["ACM_B_MSF_V_Ural_Ammo", 10],

    // Helicopters
    ["ACM_B_MSF_V_bo105p1m", 16],
    ["ACM_B_MSF_V_bo105p1m_swooper", 20],
    ["ACM_B_MSF_V_bo105p1m_Attack", 30],

    // Men
    ["ACM_MSF_Soldier_Rifleman", 1],
    ["ACM_MSF_Soldier_Medic", 1],
    ["ACM_MSF_Soldier_Radioman", 1.2],
    ["ACM_MSF_Soldier_Pioneer", 1],
    ["ACM_MSF_Soldier_Marksman", 1.1],
    ["ACM_MSF_Soldier_Autorifleman", 1.3],
    ["ACM_MSF_Soldier_Grenadier", 1.5],
    ["ACM_MSF_Soldier_rifleman_at", 3],
    ["ACM_MSF_Soldier_Anti_Tank", 5],
    ["ACM_MSF_Soldier_Anti_Tank_Assistant", 1],
    ["ACM_MSF_Soldier_Anti_Air", 10],
    ["ACM_MSF_Soldier_Squad_Leader", 1.2],
    ["ACM_MSF_Soldier_Officer", 20],
    ["ACM_MSF_Soldier_Crewman", 0.5],
    ["ACM_MSF_Soldier_HeliPilot", 0.5],

    // Men (Coast Guard)
    ["ACM_MSF_CG_Soldier_Rifleman", 2],
    ["ACM_MSF_CG_Soldier_CombatMedic", 2],
    ["ACM_MSF_CG_Soldier_RAdio_Operator", 2.2],
    ["ACM_MSF_CG_Soldier_Marksman", 2.1],
    ["ACM_MSF_CG_Soldier_Autorifleman", 2.3],
    ["ACM_MSF_CG_Soldier_Grenadier", 2.5],
    ["ACM_MSF_CG_Soldier_Rifleman_AT", 4],
    ["ACM_MSF_CG_Soldier_AntiTank", 6],
    ["ACM_MSF_CG_Soldier_AntiAir", 11],
    ["ACM_MSF_CG_Soldier_Team_Lead", 2.2],
    ["ACM_MSF_CG_Soldier_Officer", 21],
    
    // Tanks
    ["ACM_B_MSF_V_Bpz", 16],
    ["ACM_B_MSF_V_T55", 25],

    // Turrets
    ["gm_dk_army_mg3_aatripod", 4], // MG3 (High)
    ["gm_ge_army_mg3_aatripod", 4], // MG3 (High)
    ["vn_o_nva_static_pk_low", 4], // PK (Low)
    ["vn_o_nva_static_pk_high", 4], // PK (High)
    ["gm_gc_army_dshkm_aatripod", 6], // DShKM (High)
    ["vn_o_nva_65_static_dshkm_high_01", 6], // DShKM (High)
    ["vn_o_nva_static_dshkm_high_02", 6.5], // DShKM (Anti-Air)
    ["vn_b_navy_static_l70mk2", 15], // Mk2 Autocannon
    ["gm_ge_army_m120", 14], // M120 Mortar
    ["gm_gc_army_spg9_tripod", 10], // SPG-9
    ["gm_ge_army_milan_launcher_tripod", 12] // LATGM
];
