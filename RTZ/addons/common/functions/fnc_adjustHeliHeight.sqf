 ...
 * helicopters, RTS-style, without opening a context-menu action first. Each
 ...
 *
 * Applies to whole helicopters from the current selection (an individually
 ...
 * round-trip are needed (mirrors FUNC(switchStance); handler registered on
 ...
 *
 * Example:
 * [1] call rtz_common_fnc_adjustHeliHeight
 *
 * Public: No
 */
...
private _helis = SELECTED_OBJECTS select {
    _x isKindOf "Helicopter"
    && {alive _x}
    && {isNull objectParent _x}
};
if (_helis isEqualTo []) exitWith {false};

private _delta = _dir * GETGVAR(heliHeightStep,20);
