...
/*
 * Author: Maxim
 * Builds one unit's cached tag entry for FUNC(unitTags) from its server packet
 ...
 *   5 flagsText  6 threatIcon  7 threatRGB  8 threatHover
 *   9 mainSepWidth  10 statusWidth  11 threatCentreUI  12 flagCentreUI
 *   13 layoutHalfWidth  14 hasContent
 *
 * Example:
 * [_packet] call rtz_selection_fnc_buildTagEntry
 *
 * Public: No
 */

// Icon geometry (ICON_FOOT / ICON_TEXT_GAP / ICON_GAP) comes from
// script_component.hpp — the same tuned set FUNC(unitTags) draws with, so the
...
// Exact on-screen widths for the icon placement / status split in the draw pass.
// Measured here (once per cache build) so the per-frame draw only reads them
// back — size changes dirty the cache, so these stay in step.
...
// cost an empty drawIcon3D every frame.
